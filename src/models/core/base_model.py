...
class BaseDeepAD(metaclass=ABCMeta):
    ...
    def _inference(self):
        ...
        merged_z = []
        for i in range(len(z_lst[0])):
            tensors_to_merge = [sublist[i] for sublist in z_lst]
            # 使用torch.cat在第一个维度上合并张量（假设您想在第一个维度上合并）
            merged_tensor = torch.cat(tensors_to_merge, dim=0)
            # 将合并后的张量添加到列表中
            merged_z.append(merged_tensor)
        z = [tensor.data.cpu().numpy() for tensor in merged_z]
