...
from .tcn import TcnResidualBlock
...
class Cross_Correlation(torch.nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout=0.1, kdim=None, vdim=None, scale=None):
        ...
        self.drop = dropout
        self.q_proj = torch.nn.Linear(d_model, kdim)
        self.k_proj = torch.nn.Linear(d_model, kdim)
        self.v_proj = torch.nn.Linear(d_model, vdim)
        self.out_proj = torch.nn.Linear(2 * vdim, d_model)
        self.n_heads = nhead
        self.scale = scale or float(kdim//nhead) ** -0.5
        self.head_dim = kdim//nhead if kdim is not None else d_model//nhead
        self.embedding = torch.nn.Embedding(self.head_dim, vdim)
    ...
    def forward(self, query, key, value):
        ...
        len_corr = torch.mean(torch.mean(cor, dim=1), dim=1)
        ...
        for i in range(top_k):
            ...
            if ind == 0 or ind == 1:
                ...
                intervals_agg = intervals_agg + tmp_corr[i] * TcnResidualBlock(2 * self.head_dim, 2 * self.head_dim, ind, stride=1, dilation=dilation, padding=(ind-1) * dilation, dropout=self.drop).cuda()(v.permute(0, 2, 1)).permute(0, 2, 1)

        intervals_agg = intervals_agg.transpose(0, 1).contiguous().view(tgt_len, bsz, -1)

        return self.out_proj(intervals_agg)
...
class TransformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation='ReLU',
                 ...
                 device=None, dtype=None, c_in=19) -> None:
        ...
        if attn == 'self_attn':
            ...
            self.attn_model = Cross_Correlation(d_model, nhead, dropout=dropout, kdim=c_in * nhead, vdim=c_in * nhead)
        else:
...
class TransformerBatchNormEncoderLayer(torch.nn.modules.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, attn='self_attn', activation="relu", c_in=19):
...
class CDTTransformerEncoder(torch.nn.Module):
    """
    Simplest classifier/regressor. Can be either regressor or classifier because the output does not include
    softmax. Concatenates final layer embeddings and uses 0s to ignore padding embeddings in final output layer.
    """

    def __init__(self, n_features, n_output=20, seq_len=30, d_model=64,
        ...
        super(CDTTransformerEncoder, self).__init__()
